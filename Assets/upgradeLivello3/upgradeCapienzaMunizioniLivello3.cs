...
public class upgradeCapienzaMunizioniLivello3 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 10)
        {
            if (i >= singleton.TerzoLivelloProduzioneEContenitoreSoldi[liv])
            {
                ...
                singleton.LivelloTerzoLivelloCapienzaContenitoreMunizioni = liv;
    ...
    {
        liv = singleton.LivelloTerzoLivelloCapienzaContenitoreMunizioni + 1;
        if (liv >= 11)
            prezzo.text = "MAX";
        else
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = singleton.TerzoLivelloContenitore[liv - 1].ToString() + " colpi magici";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = singleton.TerzoLivelloContenitore[liv - 1].ToString() + " magic shots";
    }
}
