...
public class upgradeMetalloLivello3 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 10)
        {
            if (i >= singleton.TerzoLivelloProduzioneEContenitoreSoldi[liv])
            {
                ...
                singleton.LivelloTerzoLivelloQuantitaProduzioneMetallo = liv;
    ...
    {
        liv = singleton.LivelloTerzoLivelloQuantitaProduzioneMetallo + 1;
        if (liv >= 11)
            prezzo.text = "MAX";
        else
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = singleton.TerzoLivelloProduzione[liv - 1].ToString() + " pietre magiche ogni 3s";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = singleton.TerzoLivelloProduzione[liv - 1].ToString() + " magic stones every 3s";
    }
}
