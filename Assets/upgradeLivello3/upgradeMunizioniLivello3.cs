...
public class upgradeMunizioniLivello3 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 10)
        {
            if (i >= singleton.TerzoLivelloProduzioneEContenitoreSoldi[liv])
            {
                ...
                singleton.LivelloTerzoLivelloQuantitaProduzioneMunizioni = liv;
    ...
    {
        liv = singleton.LivelloTerzoLivelloQuantitaProduzioneMunizioni + 1;
        if (liv >= 11)
            prezzo.text = "MAX";
        else
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = singleton.TerzoLivelloProduzione[liv - 1].ToString() + " colpi magici ogni 3s";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = singleton.TerzoLivelloProduzione[liv - 1].ToString() + " magic shots every 3s";
    }
}
