...
public class upgradeCapienzaLiquidoLivello3 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 10)
        {
            if (i >= singleton.TerzoLivelloProduzioneEContenitoreSoldi[liv])
            {
                ...
                singleton.LivelloTerzoLivelloCapienzaContenitoreElisir = liv;
    ...
    {
        liv = singleton.LivelloTerzoLivelloCapienzaContenitoreElisir + 1;
        if (liv >= 11)
            prezzo.text = "MAX";
        else
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = singleton.TerzoLivelloContenitore[liv - 1].ToString() + " Elisir";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = singleton.TerzoLivelloContenitore[liv - 1].ToString() + " Elixir";
