...
public class upgradeTorrettaLivello3 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 5)
        {
            if (i >= singleton.TerzoLivelloSpawnEDannoSoldi[liv])
            {
                singleton.removeMoneta(singleton.TerzoLivelloSpawnEDannoSoldi[liv]);

                //segnare che è stato comprato l'upgrade (assegno direttamente liv visto che è il livello corrente +1)
                singleton.LivelloTerzoLivelloDannoMitraglietta = liv;
    ...
    {
        liv = singleton.LivelloTerzoLivelloDannoMitraglietta + 1;
        if (liv >= 6)
            prezzo.text = "MAX";
        else
            prezzo.text = FormatValueWithK(singleton.TerzoLivelloSpawnEDannoSoldi[liv]);
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = "Danno " + singleton.TerzoLivelloTorretta[liv - 1].ToString() + " ogni 3s";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = "Damage " + singleton.TerzoLivelloTorretta[liv - 1].ToString() + " every 3s";
    }
}
