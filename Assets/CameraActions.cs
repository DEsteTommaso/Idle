...
namespace CameraActions
{
    public class PanPinchCameraMovement : MonoBehaviour
    {
        ...
        [SerializeField] private float _orthoMin = 5;

        [Header("Maximum orthographic size")]
        [SerializeField] private float _orthoMax = 20f;
        ...
        //ho creato questo metodo per evitare di affaticare troppo con Update()
        public void refresh()
        {
            if (singleton.CompratoQuartoLivelloNuovoLivello)
                //QUANDO HO CINQUE LIVELLI
                _limitYMin = -40;
            else if (singleton.CompratoTerzoLivelloNuovoLivello)
                //QUANDO HO QUATTRO LIVELLI
                _limitYMin = -35;
            else if (singleton.CompratoSecondoLivelloNuovoLivello)
            {
                //QUANDO HO TRE LIVELLI
                _limitYMin = -26;
                _orthoMax = 20f;
            }
            else if (singleton.CompratoPrimoLivelloNuovoLivello)
            {
                //QUANDO HO DUE LIVELLI
                _limitYMin = -17;
                _orthoMax = 15f;
            }
