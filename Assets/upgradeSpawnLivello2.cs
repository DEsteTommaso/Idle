...
public class upgradeSpawnLivello2 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 5)
        {
            if (i >= singleton.SecondoLivelloSpawnEDannoSoldi[liv])
            {
                ...
                singleton.LivelloSecondoLivelloVelocitaSpawn = liv;
    ...
    {
        liv = singleton.LivelloSecondoLivelloVelocitaSpawn + 1;
        if (liv >= 6)
            prezzo.text = "MAX";
        else
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = "Spawn ogni " + singleton.SecondoLivelloSpawn[liv - 1].ToString() + "s";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = "Spawn every " + singleton.SecondoLivelloSpawn[liv - 1].ToString() + "s";
    }
}
