...
public class upgradeSpawnLivello1 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 5)
        {
            if (i >= singleton.PrimoLivelloSpawnEDannoSoldi[liv])
            {
                singleton.removeMoneta(singleton.PrimoLivelloSpawnEDannoSoldi[liv]);

                //segnare che è stato comprato l'upgrade (assegno direttamente liv visto che è il livello corrente +1)
                singleton.LivelloPrimoLivelloVelocitaSpawn = liv;
    ...
    {
        liv = singleton.LivelloPrimoLivelloVelocitaSpawn + 1;
        if (liv >= 6)
            prezzo.text = "MAX";
        else
            prezzo.text = FormatValueWithK(singleton.PrimoLivelloSpawnEDannoSoldi[liv]);
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = "Spawn ogni " + singleton.PrimoLivelloSpawn[liv - 1].ToString() + "s";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = "Spawn every " + singleton.PrimoLivelloSpawn[liv - 1].ToString() + "s";
    }
}
