...
public class torretta3_scena1 : MonoBehaviour
{
    ...
    private void sparare()
    {
        if (mostro != null)
        {
            if (singleton.Munizione - singleton.LivelloQuartoLivelloDannoMitraglietta >= 0)
            {
                ...
                testoModificabile.GetComponent<TextMeshPro>().text = "-" + singleton.LivelloQuartoLivelloDannoMitraglietta;
                StartCoroutine(wait());

                singleton.removeMunizione(singleton.LivelloQuartoLivelloDannoMitraglietta);

                GameObject newpallottola = Instantiate(pallottola, cannaArma.transform.position, Quaternion.identity);
                newpallottola.GetComponent<pallottola>().setTarget(mostro, singleton.QuartoLivelloTorretta[singleton.LivelloQuartoLivelloDannoMitraglietta], "torretta3");
            }
