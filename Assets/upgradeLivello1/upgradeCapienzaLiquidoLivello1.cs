...
public class upgradeCapienzaLiquidoLivello1 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 10)
        {
            if (i >= singleton.PrimoLivelloProduzioneEContenitoreSoldi[liv])
            {
                ...
                singleton.LivelloPrimoLivelloCapienzaContenitoreElisir = liv;
    ...
    {
        liv = singleton.LivelloPrimoLivelloCapienzaContenitoreElisir + 1;
        if (liv >= 11)
            prezzo.text = "MAX";
        else
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = singleton.PrimoLivelloContenitore[liv - 1].ToString() + " Elisir";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = singleton.PrimoLivelloContenitore[liv - 1].ToString() + " Elixir";
    }
}
