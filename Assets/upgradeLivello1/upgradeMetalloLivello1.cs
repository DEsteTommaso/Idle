...
public class upgradeMetalloLivello1 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 10)
        {
            if (i >= singleton.PrimoLivelloProduzioneEContenitoreSoldi[liv])
            {
                ...
                singleton.LivelloPrimoLivelloQuantitaProduzioneMetallo = liv;
    ...
    {
        liv = singleton.LivelloPrimoLivelloQuantitaProduzioneMetallo + 1;
        if (liv >= 11)
            prezzo.text = "MAX";
        else
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = singleton.PrimoLivelloProduzione[liv - 1].ToString() + " Pietre magiche ogni 3s";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = singleton.PrimoLivelloProduzione[liv - 1].ToString() + " Magic stones every 3s";
    }
}
