...
public class upgradeCapienzaMunizioniLivello1 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 10)
        {
            if (i >= singleton.PrimoLivelloProduzioneEContenitoreSoldi[liv])
            {
                ...
                singleton.LivelloPrimoLivelloCapienzaContenitoreMunizioni = liv;
    ...
    {
        liv = singleton.LivelloPrimoLivelloCapienzaContenitoreMunizioni + 1;
        if (liv >= 11)
            prezzo.text = "MAX";
        else
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = singleton.PrimoLivelloContenitore[liv - 1].ToString() + " colpi magici";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = singleton.PrimoLivelloContenitore[liv - 1].ToString() + " magic shots";
    }
}
