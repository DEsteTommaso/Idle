...
public class upgradeElisirLivello1 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 10)
        {
            if (i >= singleton.PrimoLivelloProduzioneEContenitoreSoldi[liv])
            {
                ...
                singleton.LivelloPrimoLivelloQuantitaEstrattoreElisir = liv;
    ...
    {
        liv = singleton.LivelloPrimoLivelloQuantitaEstrattoreElisir + 1;
        if (liv >= 11)
            prezzo.text = "MAX";
        else
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = singleton.PrimoLivelloProduzione[liv - 1].ToString() + " Elisir ogni 3s";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = singleton.PrimoLivelloProduzione[liv - 1].ToString() + " Elixir every 3s";
    }
}
