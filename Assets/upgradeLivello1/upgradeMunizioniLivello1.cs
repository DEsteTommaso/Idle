...
public class upgradeMunizioniLivello1 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 10)
        {
            if (i >= singleton.PrimoLivelloProduzioneEContenitoreSoldi[liv])
            {
                singleton.removeMoneta(singleton.PrimoLivelloProduzioneEContenitoreSoldi[liv]);

                //segnare che è stato comprato l'upgrade (assegno direttamente liv visto che è il livello corrente +1)
                singleton.LivelloPrimoLivelloQuantitaProduzioneMunizioni = liv;
    ...
    {
        liv = singleton.LivelloPrimoLivelloQuantitaProduzioneMunizioni + 1;
        if (liv >= 11)
            prezzo.text = "MAX";
        else
            prezzo.text = FormatValueWithK(singleton.PrimoLivelloProduzioneEContenitoreSoldi[liv]);
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = singleton.PrimoLivelloProduzione[liv - 1].ToString() + " Colpi magici ogni 3s";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = singleton.PrimoLivelloProduzione[liv - 1].ToString() + " Magic shots every 3s";
    }
}
