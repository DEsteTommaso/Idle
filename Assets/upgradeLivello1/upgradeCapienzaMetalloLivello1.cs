...
public class upgradeCapienzaMetalloLivello1 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 10)
        {
            if (i >= singleton.PrimoLivelloProduzioneEContenitoreSoldi[liv])
            {
                ...
                singleton.LivelloPrimoLivelloCapienzaProduzioneMetallo = liv;
    ...
    {
        liv = singleton.LivelloPrimoLivelloCapienzaProduzioneMetallo + 1;
        if (liv >= 11)
            prezzo.text = "MAX";
        else
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = singleton.PrimoLivelloContenitore[liv - 1].ToString() + " Pietre magiche";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = singleton.PrimoLivelloContenitore[liv - 1].ToString() + " Magic stones";
    }
}
