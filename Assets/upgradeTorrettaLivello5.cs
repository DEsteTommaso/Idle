...
public class upgradeTorrettaLivello5 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 5)
        {
            if (i >= singleton.QuintoLivelloSpawnEDannoSoldi[liv])
            {
                singleton.removeMoneta(singleton.QuintoLivelloSpawnEDannoSoldi[liv]);

                //segnare che è stato comprato l'upgrade (assegno direttamente liv visto che è il livello corrente +1)
                singleton.LivelloQuintoLivelloDannoMitraglietta = liv;
    ...
    {
        liv = singleton.LivelloQuintoLivelloDannoMitraglietta + 1;
        if (liv >= 6)
            prezzo.text = "MAX";
        else
            prezzo.text = FormatValueWithK(singleton.QuintoLivelloSpawnEDannoSoldi[liv]);
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = "Danno " + singleton.QuintoLivelloTorretta[liv - 1].ToString() + " ogni 3s";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = "Damage " + singleton.QuintoLivelloTorretta[liv - 1].ToString() + " every 3s";
    }
}
