...
public class upgradeCapienzaMetalloLivello4 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 10)
        {
            if (i >= singleton.QuartoLivelloProduzioneEContenitoreSoldi[liv])
            {
                ...
                singleton.LivelloQuartoLivelloCapienzaProduzioneMetallo = liv;
    ...
    {
        liv = singleton.LivelloQuartoLivelloCapienzaProduzioneMetallo + 1;
        if (liv >= 11)
            prezzo.text = "MAX";
        else
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = singleton.QuartoLivelloContenitore[liv - 1].ToString() + " pietre magiche";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = singleton.QuartoLivelloContenitore[liv - 1].ToString() + " magic stones";
