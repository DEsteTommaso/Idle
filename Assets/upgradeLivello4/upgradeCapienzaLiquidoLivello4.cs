...
public class upgradeCapienzaLiquidoLivello4 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 10)
        {
            if (i >= singleton.QuartoLivelloProduzioneEContenitoreSoldi[liv])
            {
                ...
                singleton.LivelloQuartoLivelloCapienzaContenitoreElisir = liv;
    ...
    {
        liv = singleton.LivelloQuartoLivelloCapienzaContenitoreElisir + 1;
        if (liv >= 11)
            prezzo.text = "MAX";
        else
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = singleton.QuartoLivelloContenitore[liv - 1].ToString() + " elisir";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = singleton.QuartoLivelloContenitore[liv - 1].ToString() + " elixir";
    }
}
