...
public class upgradeCapienzaMunizioniLivello4 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 10)
        {
            if (i >= singleton.QuartoLivelloProduzioneEContenitoreSoldi[liv])
            {
                ...
                singleton.LivelloQuartoLivelloCapienzaContenitoreMunizioni = liv;
    ...
    {
        liv = singleton.LivelloQuartoLivelloCapienzaContenitoreMunizioni + 1;
        if (liv >= 11)
            prezzo.text = "MAX";
        else
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = singleton.QuartoLivelloContenitore[liv - 1].ToString() + " colpi magici";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = singleton.QuartoLivelloContenitore[liv - 1].ToString() + " magic shots";
    }
}
