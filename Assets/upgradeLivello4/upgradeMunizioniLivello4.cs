...
public class upgradeMunizioniLivello4 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 10)
        {
            if (i >= singleton.QuartoLivelloProduzioneEContenitoreSoldi[liv])
            {
                singleton.removeMoneta(singleton.QuartoLivelloProduzioneEContenitoreSoldi[liv]);


                //segnare che è stato comprato l'upgrade (assegno direttamente liv visto che è il livello corrente +1)
                singleton.LivelloQuartoLivelloQuantitaProduzioneMunizioni = liv;
    ...
    {
        liv = singleton.LivelloQuartoLivelloQuantitaProduzioneMunizioni + 1;
        if (liv >= 11)
            prezzo.text = "MAX";
        else
            prezzo.text = FormatValueWithK(singleton.QuartoLivelloProduzioneEContenitoreSoldi[liv]);
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = singleton.QuartoLivelloProduzione[liv - 1].ToString() + " colpi magici ogni 3s";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = singleton.QuartoLivelloProduzione[liv - 1].ToString() + " magic shots every 3s";
    }
}
