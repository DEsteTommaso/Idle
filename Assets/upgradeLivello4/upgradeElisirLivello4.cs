...
public class upgradeElisirLivello4 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 10)
        {
            if (i >= singleton.QuartoLivelloProduzioneEContenitoreSoldi[liv])
            {
                ...
                singleton.LivelloQuartoLivelloQuantitaEstrattoreElisir = liv;
    ...
    {
        liv = singleton.LivelloQuartoLivelloQuantitaEstrattoreElisir + 1;
        if (liv >= 11)
            prezzo.text = "MAX";
        else
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = singleton.QuartoLivelloProduzione[liv - 1].ToString() + " elisir ogni 3s";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = singleton.QuartoLivelloProduzione[liv - 1].ToString() + " elixir every 3s";
    }
}
