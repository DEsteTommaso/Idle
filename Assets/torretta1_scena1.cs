...
public class torretta1_scena1 : MonoBehaviour
{
    ...
    private void sparare()
    {
        if (mostro != null)
        {
            ...
            if (singleton.Munizione - singleton.LivelloSecondoLivelloDannoMitraglietta >= 0)
            {
                ...
                testoModificabile.GetComponent<TextMeshPro>().text = "-" + singleton.LivelloSecondoLivelloDannoMitraglietta;
                StartCoroutine(wait());

                singleton.removeMunizione(singleton.LivelloSecondoLivelloDannoMitraglietta);

                GameObject newpallottola = Instantiate(pallottola, cannaArma.transform.position, Quaternion.identity);
                newpallottola.GetComponent<pallottola>().setTarget(mostro, singleton.SecondoLivelloTorretta[singleton.LivelloSecondoLivelloDannoMitraglietta],  "torretta1");
