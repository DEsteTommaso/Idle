...
public class upgradeSpawnLivello5 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 5)
        {
            if (i >= singleton.QuintoLivelloSpawnEDannoSoldi[liv])
            {
                ...
                singleton.LivelloQuintoLivelloVelocitaSpawn = liv;
    ...
    {
        liv = singleton.LivelloQuintoLivelloVelocitaSpawn + 1;
        if (liv >= 6)
            prezzo.text = "MAX";
        else
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = "Spawn ogni " + singleton.QuintoLivelloSpawn[liv - 1].ToString() + "s";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = "Spawn every " + singleton.QuintoLivelloSpawn[liv - 1].ToString() + "s";
    }
}
