...
public class upgradeSpawnLivello3 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 5)
        {
            if (i >= singleton.TerzoLivelloSpawnEDannoSoldi[liv])
            {
                ...
                singleton.LivelloTerzoLivelloVelocitaSpawn = liv;
    ...
    {
        liv = singleton.LivelloTerzoLivelloVelocitaSpawn + 1;
        if (liv >= 6)
            prezzo.text = "MAX";
        else
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = "Spawn ogni " + singleton.TerzoLivelloSpawn[liv - 1].ToString() + "s";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = "Spawn every " + singleton.TerzoLivelloSpawn[liv - 1].ToString() + "s";
    }
}
