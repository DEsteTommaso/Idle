...
public class upgradeMunizioniLivello2 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 10)
        {
            if (i >= singleton.SecondoLivelloProduzioneEContenitoreSoldi[liv])
            {
                singleton.removeMoneta(singleton.SecondoLivelloProduzioneEContenitoreSoldi[liv]);


                //segnare che è stato comprato l'upgrade (assegno direttamente liv visto che è il livello corrente +1)
                singleton.LivelloSecondoLivelloQuantitaProduzioneMunizioni = liv;
    ...
    {
        liv = singleton.LivelloSecondoLivelloQuantitaProduzioneMunizioni + 1;
        if (liv >= 11)
            prezzo.text = "MAX";
        else
            prezzo.text = FormatValueWithK(singleton.SecondoLivelloProduzioneEContenitoreSoldi[liv]);
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = singleton.SecondoLivelloProduzione[liv - 1].ToString() + " colpi magici ogni 3s";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = singleton.SecondoLivelloProduzione[liv - 1].ToString() + " magic shots every 3s";
