...
public class upgradeElisirLivello2 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 10)
        {
            if (i >= singleton.SecondoLivelloProduzioneEContenitoreSoldi[liv])
            {
                ...
                singleton.LivelloSecondoLivelloQuantitaEstrattoreElisir = liv;
    ...
    {
        liv = singleton.LivelloSecondoLivelloQuantitaEstrattoreElisir + 1;
        if (liv >= 11)
            prezzo.text = "MAX";
        else
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = singleton.SecondoLivelloProduzione[liv - 1].ToString() + " elisir ogni 3s";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = singleton.SecondoLivelloProduzione[liv - 1].ToString() + " elixir every 3s";
    }
}
