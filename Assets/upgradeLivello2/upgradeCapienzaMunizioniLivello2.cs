...
public class upgradeCapienzaMunizioniLivello2 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 10)
        {
            if (i >= singleton.SecondoLivelloProduzioneEContenitoreSoldi[liv])
            {
                ...
                singleton.LivelloSecondoLivelloCapienzaContenitoreMunizioni = liv;
    ...
    {
        liv = singleton.LivelloSecondoLivelloCapienzaContenitoreMunizioni + 1;
        if (liv >= 11)
            prezzo.text = "MAX";
        else
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = singleton.SecondoLivelloContenitore[liv - 1].ToString() + " Colpi magici";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = singleton.SecondoLivelloContenitore[liv - 1].ToString() + " Magic shots";
    }
}
