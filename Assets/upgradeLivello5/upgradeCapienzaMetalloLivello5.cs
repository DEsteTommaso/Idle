...
public class upgradeCapienzaMetalloLivello5 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 10)
        {
            if (i >= singleton.QuintoLivelloProduzioneEContenitoreSoldi[liv])
            {
                ...
                singleton.LivelloQuintoLivelloCapienzaProduzioneMetallo = liv;
    ...
    {
        liv = singleton.LivelloQuintoLivelloCapienzaProduzioneMetallo + 1;
        if (liv >= 11)
            prezzo.text = "MAX";
        else
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = singleton.QuintoLivelloContenitore[liv - 1].ToString() + " pietre magiche";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = singleton.QuintoLivelloContenitore[liv - 1].ToString() + " magis stones";
    }
}
