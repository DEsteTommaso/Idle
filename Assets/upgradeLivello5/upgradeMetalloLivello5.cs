...
public class upgradeMetalloLivello5 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 10)
        {
            if (i >= singleton.QuintoLivelloProduzioneEContenitoreSoldi[liv])
            {
                ...
                singleton.LivelloQuintoLivelloQuantitaProduzioneMetallo = liv;
    ...
    {
        liv = singleton.LivelloQuintoLivelloQuantitaProduzioneMetallo + 1;
        if (liv >= 11)
            prezzo.text = "MAX";
        else
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = singleton.QuintoLivelloProduzione[liv - 1].ToString() + " pietre magiche ogni 3s";
        else if (singleton.Lingua.Equals("en"))
