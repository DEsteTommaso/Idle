...
public class upgradeCapienzaLiquidoLivello5 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 10)
        {
            if (i >= singleton.QuintoLivelloProduzioneEContenitoreSoldi[liv])
            {
                ...
                singleton.LivelloQuintoLivelloCapienzaContenitoreElisir = liv;
    ...
    {
        liv = singleton.LivelloQuintoLivelloCapienzaContenitoreElisir + 1;
        if (liv >= 11)
            prezzo.text = "MAX";
        else
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = singleton.QuintoLivelloContenitore[liv - 1].ToString() + " Elisir";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = singleton.QuintoLivelloContenitore[liv - 1].ToString() + " Elixir";
    }
}
