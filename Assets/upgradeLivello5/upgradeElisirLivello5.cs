...
public class upgradeElisirLivello5 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 10)
        {
            if (i >= singleton.QuintoLivelloProduzioneEContenitoreSoldi[liv])
            {
                ...
                singleton.LivelloQuintoLivelloQuantitaEstrattoreElisir = liv;
    ...
    {
        liv = singleton.LivelloQuintoLivelloQuantitaEstrattoreElisir + 1;
        if (liv >= 11)
            prezzo.text = "MAX";
        else
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = singleton.QuintoLivelloProduzione[liv - 1].ToString() + " Elisir ogni 3s";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = singleton.QuintoLivelloProduzione[liv - 1].ToString() + " Elixir every 3s";
    }
}
