...
public class upgradeMunizioniLivello5 : MonoBehaviour
{
    public Text livello;
    public Text prezzo;
    ...
    private void HandleButtonClick()
    {
        ...
        if (liv <= 10)
        {
            if (i >= singleton.QuintoLivelloProduzioneEContenitoreSoldi[liv])
            {
                ...
                singleton.LivelloQuintoLivelloQuantitaProduzioneMunizioni = liv;
    ...
    {
        liv = singleton.LivelloQuintoLivelloQuantitaProduzioneMunizioni + 1;
        if (liv >= 11)
            prezzo.text = "MAX";
        else
        ...
        if (singleton.Lingua.Equals("it"))
            frequenza.text = singleton.QuintoLivelloProduzione[liv - 1].ToString() + " colpi magici ogni 3s";
        else if (singleton.Lingua.Equals("en"))
            frequenza.text = singleton.QuintoLivelloProduzione[liv - 1].ToString() + " magic shots every 3s";
    }
}
