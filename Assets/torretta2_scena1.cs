...
public class torretta2_scena1 : MonoBehaviour
{
    ...
    private void sparare()
    {
        if (mostro != null)
        {
            if (singleton.Munizione - singleton.LivelloTerzoLivelloDannoMitraglietta >= 0)
            {
                //suono sparare
                    if (audioSparare != null)
                    audioSparare.PlayOneShot(clip, singleton.Suono);

                testoModificabile.GetComponent<TextMeshPro>().text = "-" + singleton.LivelloTerzoLivelloDannoMitraglietta;
                StartCoroutine(wait());

                singleton.removeMunizione(singleton.LivelloTerzoLivelloDannoMitraglietta);

                GameObject newpallottola = Instantiate(pallottola, cannaArma.transform.position, Quaternion.identity);
                newpallottola.GetComponent<pallottola>().setTarget(mostro, singleton.TerzoLivelloTorretta[singleton.LivelloTerzoLivelloDannoMitraglietta], "torretta2");
            }
